...
def int16_from_two_bytes(bytes_pair):
    buffer = array.array("B",data_bytes).tostring()    
    bit_string = bitstring.ConstBitStream(bytes=buffer)
    return bit_string.intle

def parse_int16_values_from_byte_sequence(ls_byte_indeces, byte_sequence):
    values = [int16_from_two_bytes(byte_sequence[index:index+2]) for index in ls_byte_indeces]
    return values
...
def parse_timestamp(timestamp_bytes):
    ...
    timestamp = time.mktime(date.timetuple()) + (day_milliseconds / 1000.0000)
    return timestamp
...
def reference_unpack_bit_packed_values(data_bytes, value_nbits, twos_complement):
    total_bit_count = len(data_bytes) * 8
